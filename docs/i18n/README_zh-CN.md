...
**为什么选 Argus？** 市面上所有桌面 MCP 都只是给 AI 一张截图和一个鼠标——然后听天由命。Argus 完全不同：它直接移植自 Anthropic 自己的 Chicago MCP——Claude Code 内置桌面控制背后的生产系统。我们把 6,300 行久经考验的逻辑——安全门控、token 预算、批量执行引擎——原封不动地拿来，只替换了 Windows 原生层。这可能是**当今世界上架构最先进的 computer-use 落地实现**，并且完全开源。
