...
**なぜ Argus なのか？** 他のデスクトップ MCP は、AI にスクリーンショットとマウスを渡して、あとは祈るだけです。Argus はまったく異なります。Anthropic 自身の Chicago MCP ── Claude Code に組み込まれたデスクトップ制御を支えるプロダクションシステム ── を直接移植したものです。セキュリティゲート、トークン予算管理、バッチ実行エンジンなど、実戦で鍛え抜かれた 6,300 行のロジックをそのまま活用し、ネイティブレイヤーだけを Windows 向けに置き換えました。これはおそらく、**現時点で世界で最もアーキテクチャ的に優れた computer-use 実装**であり、オープンソースとして公開されています。
